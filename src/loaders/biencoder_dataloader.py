...
class SyntRetDataLoader:
    ...
    def _transform_func(self, examples: Dict[str, List]) -> Dict[str, List]:
        current_epoch = int(self.trainer.state.epoch or 0)
        merged_docs = [doc for pair in zip(examples['pos'], examples['neg']) for doc in pair]

        query_batch_dict = self.tokenizer(examples['q'],
                                          max_length=self.args.q_max_len,
                                          padding=PaddingStrategy.DO_NOT_PAD,
                                          truncation=True)
        doc_batch_dict = self.tokenizer(merged_docs,
                                        max_length=self.args.p_max_len,
                                        padding=PaddingStrategy.DO_NOT_PAD,
                                        truncation=True)
    ...
    def _get_transformed_datasets(self) -> Tuple:
        ...
        if self.args.train_tasks != 'all':
            tasks = self.args.train_tasks.strip().split('+')
            raw_dataset = raw_dataset.filter(lambda x: x['task_type'] in tasks)
        if self.args.max_train_samples is not None:
        ...
        raw_dataset.set_transform(self._transform_func)
        split_data = raw_dataset.train_test_split(test_size=25000, shuffle=True, seed=42)
